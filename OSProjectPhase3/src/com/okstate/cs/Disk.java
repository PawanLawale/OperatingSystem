...
public class Disk {
	...
	/*
	 * Releases the frames occupied by the given job.
	 */
	public static void releaseDiskForJob(String jobId){
		DiskSMT smt = diskJobStorage.get(jobId);
		if(smt != null){
			DiskPMT[] seg = smt.segment;
			for(int i=0;i<3;i++){
				if(seg[i] != null){
					for(String key : seg[i].pageFrameMap.keySet()){
						String address = seg[i].pageFrameMap.get(key);
						int frameAdd = Integer.parseInt(address, 16);
						int frameNo = frameAdd/8;
						memory_fmbv[frameNo] = 0;
						int add = (frameNo*8);
						for(int k=add;k<add+8;k++){
							memory.put(Utility.decimalToHex(k, 3), null);
						}
					}
				}
			}
		}
		diskJobStorage.remove(jobId);
	}
	
	/*
	 * Returns the number of available Frames.
	 */
	public static int memoryUtilization(){
		int totalFreeFrames = 0;
		for(int i=0;i<memory_fmbv.length;i++){
			if(memory_fmbv[i] == 0){
				totalFreeFrames++;
			}
		}
		return totalFreeFrames;
	}
}
